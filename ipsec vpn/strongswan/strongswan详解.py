...
策略路由
    ...
    像如ping 127.0.0.1，则会首先在local表（系统维护）中看有没有相应规则，
    在第3条时命中，根据记录的dev lo，得知将该包发往本地回环
    可用的规则从最高优先级（0）到最低优先级（32767）进行扫描，
    一旦 RPDB 策略匹配，就会使用指定的表来查找下一跳
    如果路由过程无法从规则所指示的路由表计算出数据包的下一跳，
    则继续处理到 RPDB 中的下一条规则（ip rule show中列出的）
    ...
    理解：策略路由跟ipsec没有必然关系，strongswan只需要告诉xfrm使用策略路由，并添加好相应的策略即可
...
2 数据流
    ...
    file://../../网络/iptables.png
    file://../../LINUX/iptables的表链结构.png
    file://../../LINUX/iptables的表链结构2.png
...
3 理解linux kernel中的sa概念和管理
    3.1 提供给用户的sa接口
        ...
        3.1.1 使用racoon(包含于ipsec-tools)配置sa  file://../ipsec tools.txt
        ...
        3.1.2 使用racoon配置policy(有关racoon，参：https://blog.csdn.net/ai58203/article/details/101469199）
    ...
    5 strongswan中的sa
        5.1 概述
            ...
            5.1.1 strongswan中的plugin
                ...
                注：
                    许多 strongSwan 组件库都附带了一组插件。
                    Libstrong swan 的插件提供了加密后端、 URI 获取器和数据库层，
                    而 libcharon 提供了一大套非常专业的插件来满足特定的需求。
                    ？？ ——>
                    charon是秘钥交互的核心实现，但strongswan不止是秘钥交互，
                    其他的功能是strongswan的插件实现的 
                strongswan插件：
                    https://docs.strongswan.org/docs/5.9/plugins/plugins.html
                    acert、aes、aesni、af-alg、agent、bliss、blowfish、botan、ccm、chapoly、
                    cmac、constraints、ctr、curl、curve25519、des、dnskey、drbg、files、
                    fips-prf、gcm、gcrypt、gmp、hmac、kdf、keychain、ldap、md4、md5、mgf1、
                    mysql、newhope、nonce、ntru、openssl、padlock、pem、pgp、pkcs1、pkcs7、
                    pkcs8、pkcs11、pkcs12、pubkey、random、rc2、rdrand、revocation、sha1、
                    sha2、sha3、soup、sqlite、sshkey、test-vectors、unbound、winhttp、
                    wolfssl、x509、xcbc
                charon的插件：
                    https://docs.strongswan.org/docs/5.9/plugins/plugins.html
                    addrblock、android-dns、android-log、attr、attr-sql、bypass-lan、
                    certexpire、connmark、counters、coupling、dhcp、dnscert、duplicheck、
                    eap-aka、eap-aka-3gpp、eap-aka-3gpp2、eap-dynamic、eap-gtc、eap-identity、
                    eap-md5、eap-mschapv2、eap-peap、eap-radius、eap-sim、eap-sim-file、
                    eap-sim-pcsc、eap-simaka-pseudonym、eap-simaka-reauth、eap-simaka-sql、
                    eap-tls、eap-tnc、eap-ttls、error-notify、ext-auth、farp、forecast、
                    ha、ipseckey、kernel-libipsec、kernel-netlink、kernel-iph、kernel-pfkey、
                    kernel-pfroute、kernel-wfp、led、load-tester、lookip、medcli、medsrv、
                    osx-attr、p-cscf、radattr、resolve、save-keys、selinux、smp、
                    socket-default、socket-dynamic、socket-win、sql、stroke、tnc-ifmap、
                    tnc-pdp、systime-fix、uci、unity、updown、vici、whitelist、
                    xauth-eap、xauth-generic、xauth-noauth、xauth-pam
        ...
        5.2 启动过程
            ...
            5.2.4 systemd的启动过程
                ...
                2. 执行swanctl --load-all --noprompt命令
                   --load-all = load credentials, authorities, pools and connections
                charon-systemd进程
        ...
        5.3 调用过程
                ...
                接口会根据具体注册的plugin调用各plugin的相应，add_sa, add_policy函数
    ...
    6 sa的抽象模型
        sa
            ...
            id		                    消息id ？
            spi		                    协商过程带过来的
            mode	    transport/tunnel传输模式	
            protocol	esp/ah/ipcom	加密协议的方式
            sip		                    源ip，另一条隧道是sip和dip互换的，故两个sa
            dip		                    目的ip
            life		                生存时间
            enc_alg		                加密算法
            enc_key		                加密秘钥
            integrity_alg		        完整性验证算法
            integrity_key		        完整性验证秘钥
            nat		                    是否做nat转换
        policy
            属性	取值	            说明
            id		
