...
NAT篇 NAT Server 基础篇
    ...
    针对服务器的地址转换，我们赋予了它一个形象的名字――NAT Server（服务器映射）
...
VPN篇 IPSec携手IKE，珠联璧合显神威
            ...
            主模式
                ...
                下面以网关A主动发起IKE协商为例进行讲解
                    ...
                    2. 使用DH算法交换密钥材料，并生成密钥
                        ...
                        最终会产生三个密钥：
                            SKEYID_a： 认证用
                                ISAKMP消息完整性验证密钥――谁也别想篡改ISAKMP消息了，
                                只要消息稍有改动，响应端完整性检查就会发现
                            SKEYID_e： 加密用
                                ISAKMP消息加密密钥――再也别想窃取ISAKMP消息了，窃取了也看不懂
                            SKEYID_d:  
                                用于衍生出IPSec报文加密和验证密钥――最终是由这个密钥保证IPSec封装的数据报文的安全性
    ...
    IPSec协议框架总结   
        ...
        加密算法（DES、3DES、AES）――IPSec报文的易容之术
