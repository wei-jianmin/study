...
class QObjectData 
 {
  public:
    ...
    uint sendChildEvents : 1;   #不 应该为true
...
class QObjectPrivate : public QObjectData
 {
  ...
  public:
   '结构定义 ：ExtraData、Connection、ConnectionList、Sender
    struct ExtraData
     {
        ExtraData() {}
        #ifndef QT_NO_USERDATA
...
QObject函数实现 #绝大多数函数实现都出自qobject.cpp
    #创建了d_ptr对象，并将当前对象的指针传给d_ptr->q_ptr
    QObject::QObject(QObject *parent) : d_ptr(new QObjectPrivate)
     {
        Q_D(QObject);
        d_ptr->q_ptr = this;    #this为对象指针，如MyObject继承自QObject，则this就是MyObject对象的指针
    ...
    void qt_addObject(QObject *)
     {
        objectCount.ref();  #objectCount是在qobject.cpp中声明的静态全局变量
     }
