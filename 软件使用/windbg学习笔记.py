...
-启动调试：
    -附加到进程
        ...
        windgb:
            ...
            当WinDbg在静止模式时，windgb File菜单/Attach to a Process（快捷键F6）
...
-必知必会
    ...
    -调试器配置
        配置要搜索的dll路径
            _NT_DEBUGGER_EXTENSION_PATH     指定搜索dll的路径的，多个目录用分号分割
        配置源码路径
            File菜单 Source File path， 多个路径用分号分隔
        配置符号文件搜索路径
            File菜单 Symbol File path， 多个路径用分号分隔
        配置要使用哪种表达式
            _NT_EXPR_EVAL = {masm | c++}    指定默认使用的求值表达式，默认masm
    -进行调试：
        附加到进程
            File菜单 Attach to a Process（快捷键F6）
        创建新进程
            File菜单 Open Executable
        调试带参数的程序
            命令行方式执行，如：windbg.exe  a.exe myname
        控制程序的执行
            F7 / CTRL+F10   : 运行到光标处
            SHIFT + F5      : 停止调试
            F5  : Go (g)    ：开始调试
    ...
    -断点管理
        ...
        普通断点（地址断点）
            临时断点 bp
                ...
                bp `source.c:31`
                     ...
                     完整的源码行号的格式如下：
                     `[[Module!]Filename][:LineNumber]`
                     如果你有多个文件同名，Filename 应该包含完整的路径和文件名
                     如果你仅提供文件名或者一部分路径，而且有多个匹配项，
                     调试器将使用第一个找到的匹配项。
                     如果省略Filename，调试器假定使用当前运行位置所在的源文件。
                     除非带有 0x 前缀，否则 LineNumber 将被读作十进制数值
                     如果省略 LineNumber，表达式将被计算为对应源文件的起始可执行地址。
             永久断点 bu
                与 bp 的使用方式一个，但功能上有如下区别：
                bp 断点的位置总是被转换成地址（如 bp MyClass::MyMethod）
                而bu断点仍然和使用的符号值关联
                断点所在模块卸载后，bp断点会自动删除，而bu断点不会
                bp断点不会保存到windbg的工作空间中，bu断点会被保存
        方法断点
            bp MyClass::MyMethod 或 bp MyClass__MyMethod  （MASM语法）
        命令断点
            可以设置在断点触发时，自动执行的语句
            方法就是在设置断点的命令后面跟上命令字符串，
            bu MyFunction+0x47 ".dump c:\mydump.dmp; g" 
        条件断点
            条件断点其实就是命令断点的特殊使用
            bp Address ".if (Condition) {OptionalCommands} .else {gc}"
            当断点触发时，在断点命令中，判断是否符合条件，如果不符合，就继续执行
            例：bp `func.cpp:18` ".if(x>=0){};.else{gc;}"   
        访问断点
            这种断点在指定内存被访问时触发，格式如下：
            ba Access Size [Address [Passes]] ["CommandString"]
            Access 访问模式
                e:当cpu执行到该地址时中断
                r:当cpu读写该地址时中断
                w:当cpu在该地址写操作时中断
            Size ：要监视的长度，以字节为单位，值可以为1、2、4、8等
            Address ： 断点地址
            Passes ： 经过多少次后，该断点才开始生效
            CommandString ： 到达断点后自动执行的指令
        查看断点
            bl 命令 或 使用断点窗口查看
        删除断点
            使用断点窗口
        ...
        说明
            断点只能在暂停调试时，才能增加或删除
            可使用 Edit菜单/BreakPoints 设置或查看断点，快捷键 alt + F9
            可使用工具栏的手形按钮快速在源文件的当前行设置断点，快捷键 F9
    -查看变量内容 
        Watch 窗口始终使用C++表达式。
        Locals 窗口始终使用C++ 表达式。
        所以在 Watch 中查看变量值，和 vs 类似
        dt 变量名 ： 显示变量的资料和结构
        !da 地址  ： 查看数组内容
    -查看内存
        使用内存监视窗口查看    
    -调试器命令行使用：
        ...
        tab补全规则：
            ...
            如果没有显式指定模块名，则用本地符号和模块来补完。
            如果给出了模块或者模块的模板，则会按所有匹配的符号来补完。
        使用.cls (Clear Screen) 命令清空调试器命令窗口的所有文本
    -表达式语法 
        很多命令和扩展命令都接受表达式作为参数。
        调试器在执行命令之前先计算这些表达式的值。
        调试器能够识别两种表达式类型：MASM表达式和C++表达式。
    ...
    -常用命令行操作
        ...
        p  : step over (p)
        t  : step into (t : trace)
        gu : step out  (gu : go up)
        进入调试状态时，直接回车可重复执行上一条命
        查看调用堆栈
        查看变量内容
        dt 变量名 ： 显示变量的资料和结构
        !da 地址  ： 查看数组内容
    ...
    -伪寄存器
        所有伪寄存器都以一个美元标记（$）打头
        你可以在 $ 标记前加一个 at 标记（@）
        告诉 MASM 后面的记号是一个寄存器或者伪寄存器
        这不是必须的，但会加快解析速度
        $dbgtime    当前时间
        $tid        当前线程的线程 ID。
        $tpid	    当前线程所在进程的进程 ID（PID）。
        $exp	    最后一个被求值的表达式。
        $t0..$t19   可存储用户表达式值的寄存器
            设置寄存器值的语法：r $t0 = 7
    -延迟符号加载
        调试器默认延迟符号加载，当用到时才加载；
        可以使用ld (Load Symbols) 命令强制加载符号
            ld ModuleName [/f FileName]
            ModuleName  指定要加载符号的模块，可以包含各种通配符和修饰符。
            /f FileName 改变选择用来匹配的名称。
                默认情况下使用模块名来匹配，
                但是当使用/f时，使用文件名来匹配而不是模块名。
                FileName 可以包含各种通配符合修饰符。
        可以使用.reload (Reload Module)和/f选项强制加载符号
            ...
            Module  指定主控机要重加载符号的目标机上的映像名。
                ...
                如果省略Module，.reload 命令的行为由使用的Options 决定。
            支持使用 -? 显示这个命令的简短帮助文本。
            例：.reload /i mytest.dll  #加载不匹配的pdb文件