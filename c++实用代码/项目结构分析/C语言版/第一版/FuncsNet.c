...
long getcallstack( void*lpEP,char buf[3][50],int* err);
...
void _LogFunc(FuncsNet *pthis,const char* info)
{
	char buf[3][50]={0};
	int err=0;
	if(pthis==NULL)
	...
	if(info==NULL)
	{
		strcpy(pthis->err_msg,"logfunc,parram error!");
		return;
	}
	if(pthis->logopened>0){
		__try
	...
	if(pthis->logopened>0)
	{
		pthis->ident=LocateFuncPos(pthis,buf[1],buf[2]);
		strcpy(pthis->names.name_buf[pthis->names.index++],buf[0]);
		memset(pthis->func_info,0,1024);
		memcpy(pthis->func_info,info,min2(1024,strlen(info)));
		if(pthis->ident>0)
		{
...
void InitialFuncsNet(FuncsNet *pthis)
{
	...
	for(i=0;i<40;i++)
	{
		memset(pthis->names.name_buf[i],0,50);
	}
}
...
long getcallstack( void*lpEP,char buf[3][50],int *err)
{
	...
	{
		strncpy(buf[0],pfunctioninfo->Name,50);
		*err=0;
	}
	else
	{
