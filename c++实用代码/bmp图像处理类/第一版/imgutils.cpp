...
//
void img_util::_init(u16 bpp,u16 w,u16 h,u8* data/*=NULL*/,u8* pallet/*=NULL*/)
{
	_data_ok=false;
	if(bpp==0 || w==0 || h==0)
		return;
	...
	if(_pallet_len>0)
	{
		...
		if(pallet)
			memcpy(_pallet,pallet,_pallet_len);
		else
...
bool img_util::reset_img_info(u16 bpp,u16 w,u16 h,u8* data/*=NULL*/,u8* pallet/*=NULL*/)
{
	_clear_data();
	_init(bpp,w,h,data,pallet);
	if(_data_ok)
...
bool img_util::setpix24( u16 r,u16 c,pix32 b24 )
{
	if(_bpp!=24)
		return false;
	u8 * p = _locate_pix(r,c);
...
bool img_util::setpix32( u16 r,u16 c,pix32 b32 )
{
	if(_bpp!=32)
		return false;
	u8 * p = _locate_pix(r,c);
...
iu::sections img_util_ex::scan_color_sections(u16 line_index, u16 lstw/*=100*/,u8 tolarance/*=5*/,bool greedymode )
{
	section s;
	sections secs;
	secs.clear();
	...
	if(_bpp==24)
	{
		...
		for(i=1;i<_w;i++)
		{
			...
			if(_pix32_similar(base_pix,this_pix,tolarance))
			{
...
void img_util_ex::_check_sections(sections& secs,u8 tolarance,u16 begin_line_index,u16 end_line_index)
{
	section s;
	...
	if(_bpp == 24)
	{
		for(int i=0;i<secs.size();i++)	//取每个区间
		{
			...
			for(int j=s.x0;j<s.x1;j++)	//反斜线扫描本区间
			{
				...
				if(_pix32_similar(this_pix,base_pix,tolarance) == false)	//发现坏点
				{
...
bool img_util_ex::_pix32_similar(pix32 p1,pix32 p2,u8 tolarance)
{
...
iu::color_blocks img_util_ex::scan_color_blocks( u16 lstw/*=100*/,u16 lsth/*=5*/,u8 tolarance/*=5*/ )
{
	bool secs_ok;
	section sec;
	color_block blk;
	...
	{
		sections secs1 = scan_color_sections(i,lstw,tolarance,false);
		sections secs2 = scan_color_sections(i+lsth-1,lstw,tolarance,true);
		sections secs = get_inter_sections(secs1,secs2,lstw);
		_check_sections(secs,tolarance,i,i+lsth-1);	//滤除有杂色的块
...
void img_util_ex::set_block_color(color_block blk,pix32 color)
{
	if(_bpp!=24 && _bpp!=32)
		return;
	if(_bpp == 24)
...
iu::pix32 img_util_ex::get_block_color( color_block blk )
{
	u32 red,grn,blu;
	u16 count=0;
	pix32 pix;
...
pix32 img_util_ex::everate_block_color( color_block blk )
{
	pix32 pix = get_block_color(blk);
	if(pix.c.r==255)
		pix.c.r=254;
	if(pix.c.g==255)
		pix.c.g=254;
...
iu::u16 img_util_ex::recognize_data()
{
	data_recoreds datas;
    i32 col,value;
    pix32 pix;
