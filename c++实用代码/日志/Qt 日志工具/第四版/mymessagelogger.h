...
*   需特别说明的是，设置最小日志等级不能小于0，最大日志等级不能超过65535，
*   设置日志范围时，格式如：0-5,100,300-65535，有效值为0~65535，范围区间按有效到大顺序排列，
*   如果设置日志范围失败，则仍使用之前成功设置过的日志范围，默认范围为0-65535
* 2.mylog(LEVEL) ：
*   mylog(LEVEL)与qDebug()用法一样，但可以指定LEVEL值，unsigned short类型，
*   只有LEVEL值在最小输出日志等级和最大输出日志等级之间，且在日志等级范围中时，该条日志才会输出
...
enum MyControlType{
    ...
    set_log_level_range,//设置日志等级输出范围，不在该范围内的日志不会输出，对应数据类型为string，默认范围为0-65535
...
private:
    ...
    static QList<QPair<int,int>> _log_level_range;
    static bool check_level_in_range(int level);
private:
