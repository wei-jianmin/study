...
static int openlog(SimpleLog* log,const char *path,const char* mode)
{
	...
	if(log->file !=NULL)
	{
...
#ifdef WRITE_FILE_INFO
		{
			...
			char path0[MAX_PATH]={0};			//模块路径名
			wchar_t path1[_MAX_PATH] = {0};		//模块路径名
			wchar_t path2[MAX_PATH]={0};		//要写入文件的信息（多字节转换前）
			wchar_t *ppath1=path1;
			wchar_t *ppath2=path2;
			char path3[MAX_PATH*2]={0};			//要写入文件的信息（多字节转换后）
			int offset=0;
			DWORD dwLen =0;
			char *pszAppVersion=NULL;
			
			fwrite("日志工具版本：",strlen("日志工具版本："),1,log->file);
			fwrite(SIMPLELOG_VER,strlen(SIMPLELOG_VER),1,log->file);
			fwrite("\n",1,1,log->file);

			GetModuleFileNameA(NULL,path0,MAX_PATH);//得到程序模块名称，全路径，多字节
			GetModuleFileNameW(NULL,path1,MAX_PATH);//得到程序模块名称，全路径，宽字节
			wcscpy(path2,L"ApplicationPath=");
			ppath2+=16;
			wcscat(path2,path1);

			offset=Cal(path1,LINEWIDTH-16);			//控制断行长度与分割线长度一致
			ppath1+=offset;
			ppath2+=offset;
			ppath2[0]='\n';
			ppath2++;
			while(ppath1 && wcslen(ppath1)>0)
			{
				wcscpy(ppath2,L"                ");
				ppath2+=16;
				wcscpy(ppath2,ppath1);
				offset=Cal(ppath1,LINEWIDTH-16);
				ppath1+=offset;
				ppath2+=offset;
				ppath2[0]='\n';
				ppath2++;
...
static int writelog(SimpleLog* log,const char *format,...)
{
	...
	vsnprintf_s(pmsg,20480,20450,format,arg_ptr);
	va_end(arg_ptr);		// 要写的内容已经存在了pmsg中

	if(log->isopen)
	{
		//if(log->last_msg.compare(pmsg)==0)		//消息发生重复
		int len=min(20480,strlen(pmsg)+1);		//获取待写文本长度，不超过20480
		char timebuf[20]={0};
		SYSTEMTIME sys; 
		...
		int size=0;
		GetLocalTime(&sys); 
		if(strncmp(log->last_msg,pmsg,len)==0)	//如果跟上次记录的消息内容一致
		{
		...
		else if(log->msg_dup_count>0)			//消息之前重复过，现在不重复了
		{
			...
			_itoa( log->msg_dup_count, buf, 15);
...
static int writebytes(SimpleLog* log,const char* pfun,const char* prefix,const char *pbytes,int len)
{
	...
	{
		//if(log->last_msg.compare(pmsg)==0)		//消息发生重复
		int len=min(20480,strlen(pmsg)+1);
		char timebuf[20]={0};
		char tmpmsgbuf[20480]={0};
		SYSTEMTIME sys; 
		unsigned int l1,l2,l3,l4;
		unsigned long dw=0;
		int size=0;
		...
		tmpmsgbuf[20470]=0;
		if(strncmp(log->last_msg,tmpmsgbuf,len)==0)
		{
			log->msg_dup_count++;				//记录消息重复次数
			if(log->msg_dup_count==1)			//消息重复一次
			{
...
static int lwritelog(SimpleLog* log,const wchar_t *wformat,...)
{
	...
	//vswprintf_s(pmsgw,40900,wformat,arg_ptr);
	_vsnwprintf_s(pmsgw,40960,40900,wformat,arg_ptr);
	va_end(arg_ptr);			// 要写的内容已经存在了pmsg中

	wcstombs(pmsg,pmsgw,20480);
...
SimpleLog SimpleLogDefault={closelog,writelog,lwritelog,writebytes,openlog,viewcallstack,dumpcallstack,NULL,0,0,0,"","",NULL,NULL};
