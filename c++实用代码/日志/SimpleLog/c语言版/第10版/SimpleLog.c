...
static int openlog(SimpleLog* log,const char *path,const char* mode)
{
	...
	if(log->file !=NULL)
	{
		...
		fwrite("\n",1,1,log->file);
		{
			...
			fwrite(szAppFullPath,strlen(szAppFullPath),1,log->file);
			//获取当前文件的版本信息
			dwLen = GetFileVersionInfoSize(szAppFullPath,NULL); 
			pszAppVersion = (char*)calloc(dwLen+1);
			...
			if(pszAppVersion)
			{
				...
				strVersion = (char*)calloc(30);
				memset(strVersion,0,30);
				if(pFileInfo)
				{
					//获取版本号
...
void MsgBoxEx(hWnd hwnd,const char* caption,const char *FormatMsg,...)
{
	...
	MessageBoxA(hwnd,pc,caption,MB_OK);
}



SimpleLog SimpleLogDefault={NULL,0,0,0,"","",NULL,closelog,writelog,lwritelog,openlog,viewcallstack,dumpcallstack};
