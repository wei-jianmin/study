...
CSimpleLog::CSimpleLog(const char *path,const char* mode)
{
	isopen=false;
	if(path == NULL)
		return;
...
bool CSimpleLog::resetpath(const char *path,const char *mode/* ="a+" */)
{
	if(path == NULL  )
		return false;
	if(isopen)
...
bool CSimpleLog::writelog(const char *pmsg)
{
	if(!isopen)
		return false;
	if(isopen)
...
bool CSimpleLog::writelogx(const char *format,...)
{
	if(!isopen)
		return false;
	if(format==NULL)
...
bool CSimpleLog::clear(const char*path)
{
	if(path==NULL)
		return false;
	int a=_access(path,6);
...
bool CSimpleLog::write(const char *path,const char *info)
{
...
bool CSimpleLog::writex(const char * path,const char * info,...)
{
	if(path==NULL)
		return false;
	char buf[2048]={0};
...
char* CSimpleLog::WcharToChar(const wchar_t* wc)//宽字节转单字节 
{
...
bool CSimpleLog::write(const wchar_t *path,const wchar_t *info)
{
...
bool CSimpleLog::writelog(const wchar_t *pmsg)
{
...
bool CSimpleLog::writex(const wchar_t * path,const wchar_t * info,...)
{
	if(path==NULL)
		return false;
	wchar_t buf[2048]={0};
...
bool CSimpleLog::writelogx(const wchar_t *format,...)
{
	if(!isopen)
		return false;
	if(format==NULL)
