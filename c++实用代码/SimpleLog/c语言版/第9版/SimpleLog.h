...
#endif
//================================================================
#define __STR2WSTR(str,s) L##str##L": "##s
#define _STR2WSTR(str,s) __STR2WSTR(str,s)
#define FUNCTIONW(s) _STR2WSTR(__FUNCTION__,s)
...
#endif
#ifndef WRITELOG
#define WRITELOG(pLog,strF,...) if(pLog)pLog->write(pLog,strF,__VA_ARGS__)
#endif
#ifndef LWRITELOG		//写日志函数(宽字节版)
#define LWRITELOG(pLog,strF,...) if(pLog)pLog->lwrite(pLog,strF,__VA_ARGS__)
#endif
#ifndef CLOSELOG
#define CLOSELOG(pLog) if(pLog)pLog->close(pLog),free(pLog),pLog=NULL
#endif
#pragma endregion
...
#define UNSET_LOGPREFIX(pLog) if(pLog)pLog->pstr_prefix=NULL
#endif
#ifndef FIX_WRITELOG
#define FIX_WRITELOG(pLog,strF,...) if(pLog)pLog->pstr_prefix=SimpleLogPrefix,pLog->prefix=1,pLog->write(pLog,strF,__VA_ARGS__)
#endif
#ifndef LSET_LOGPREFIX
#define LSET_LOGPREFIX(pLog,str) wchar_t* SimpleLogPrefixL=str
#endif
#ifndef LUNSET_LOGPREFIX
#define LUNSET_LOGPREFIX(pLog) if(pLog)pLog->pstr_prefix_l=NULL
#endif
#ifndef LFIX_WRITELOG
#define LFIX_WRITELOG(pLog,strF,...) if(pLog)pLog->pstr_prefix_l=SimpleLogPrefixL,pLog->prefix=1,pLog->lwrite(pLog,strF,__VA_ARGS__)
#endif
#pragma endregion
...
#define WRITELOG2(pLog,strF,...) if(pLog)pLog->write(pLog,"!!"##__FUNCTION__##": "##strF,__VA_ARGS__)
#endif
#ifndef LWRITELOG2	//写日志函数(宽字节版)，自动添加所在函数名
#define LWRITELOG2(pLog,strF,...) if(pLog)pLog->lwrite(pLog,L"!!"##FUNCTIONW(strF),__VA_ARGS__)
#endif
#ifndef VIEWCALL	//查看调用堆栈函数，depth指定查看调用堆栈深度
#define VIEWCALL(pLog,depth) if(pLog)pLog->viewcallstack(pLog,depth)
#endif
#ifndef DUMPCALL	//查看调用堆栈函数，配合SEH使用，depth指定查看调用堆栈深度
#define DUMPCALL(pLog,depth) if(pLog)pLog->dumpcallstack( pLog,depth,GetExceptionInformation())
#endif
#else
#ifndef msgbox2
...
#endif
#ifndef WRITELOG2
#define WRITELOG2(pLog,strF,...) if(pLog)pLog->write(pLog,strF,__VA_ARGS__)
#endif
#ifndef VIEWCALL
#define VIEWCALL(pLog,depth) if(pLog)pLog->write(pLog,"不是VS编译器,无法查看函数调用堆栈")
#endif
#ifndef DUMPCALL
#define DUMPCALL(pLog,depth) if(pLog)pLog->dumpcallstack(pLog,depth,0)
#endif
#endif
#pragma endregion
