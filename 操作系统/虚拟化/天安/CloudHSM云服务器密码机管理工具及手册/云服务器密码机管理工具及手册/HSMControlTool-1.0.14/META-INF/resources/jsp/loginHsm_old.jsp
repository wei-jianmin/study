   		...
   		<form class="form-horizontal" style="height: 180px; padding-top:-20px;" id="loginForm" action="${ctx}/login.html" method="post">
       		<div class="form-group">
   				<div class="col-sm-12" >
       				...
       				<div class="input-group" style="margin-top:10px;">
						是否认证：
						 	<input type="radio" name="isAuthentication" id="isAuthentication0" value="0" />是
						 	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" name="isAuthentication"  checked="checked"  id="isAuthentication1" value="1" />否
					</div>
					<div class="row cl" id="privateKeyDivId" style="display:none;margin-top:10px;">
						...
						<div class="formControls col-xs-8 col-sm-9">
							<input class="form-control" type="text" placeholder="请输入认证私钥" id="privateKey" name="privateKey" value=""/>	
						</div>
					</div>
					<div class="row cl" id="publicKeyDivId" style="display:none;margin-top:10px;">
						<label class="form-label col-xs-4 col-sm-3">
							<span class="input-group-addon" style="border-top-left-radius:4px;border-bottom-left-radius:4px;display:block;width:68px;margin-top:-3px;">
							<span class="glyphicon" style="height:20px;">
								公钥:
						...
						<div class="formControls col-xs-8 col-sm-9">
							<input class="form-control" type="text" placeholder="请输入认证公钥" id="publicKey" name="publicKey" value=""/>	
						</div>
					</div>
...
$(function(){
 	showCont();
 	$("input[name=isAuthentication]").click(function(){
  		showCont();
 	});
});
function showCont(){
 	switch($("input[name=isAuthentication]:checked").attr("id")){
	  	case "isAuthentication0":
		   	$("#privateKeyDivId").css("display","block");
		   	$("#publicKeyDivId").css("display","block");
	   		break;
	  	case "isAuthentication1":
	  		$("#privateKeyDivId").css("display","none");
	  		$("#publicKeyDivId").css("display","none");
		   	break;
  			default:
	   	break;
 	}
...
function login() {
	...
	var isAuthentication = $("input[name=isAuthentication]:checked").attr("id");
	if(isAuthentication=="isAuthentication0"){
...
function login2(){
	// 数据已经校验完毕
	var hsmIp = $("#hsmIp").val().trim();
	var authentication = $("input[name=isAuthentication]:checked").attr("id");
	if(authentication=="isAuthentication0"){
		authentication=true;
	}else{
	...
	$.ajax({
        ...
        success: function(data) {
        	...
        	if(result == "yes"){ // 登录通过
    			...
    			layer.msg("HSM IP或认证公私钥不能为空",{icon:7,time:2000});
    			changeImg();
