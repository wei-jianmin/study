...
密钥对保护方法
    ...
    1. 产生一个对称密钥
    2. 按对称密码算法标识指定的算法，对sm2私钥进行加密，得到私钥的密文。
       若对称算法为分组算法，则其运算模式为ECB
    3. 使用外部sm2光耀加密对称密钥得到对称密钥密文
...
详解国密SM2的加密和解密                
    https://blog.csdn.net/mogoweb/article/details/105108689
    如果现有的网络库中已经实现ECC算法，只需加入SM2命名曲线的参数即可
    这对于ECDHE密钥协商和ECDSA数字签名这两种用途而言确实是足够的
    现有的网络库，很少将ECC算法直接用于加密和解密
    但在实现ECC_SM4_SM3这个密码套件中，
    在密钥交换过程中存在客户端将 Pre-Master Secret 使用 SM2 公钥加密后
    传给服务器端的步骤，所以我们需要实现 SM2 的加密和解密。
    如何使用 SM2 算法进行加密和解密，可以参考的资料是
    《GMT 0003.4-2012 SM2椭圆曲线公钥密码算法第4部分：公钥加密算法》
    但这份文档有些地方描述的并不是很清楚，实现起来也是很头疼
    在参考了GmSSL开源项目的源码之后，总算把这个过程梳理清楚了。
    先总结一下，把其中容易出错的位置重点说明一下，供大家参考。
    。。。 略，具体参看源链接
