...
ARP
    ...
    ARP响应
        ...
        开启arp广播的路由器，查看目标IP地址，
        如果不是自己的ip（接收端口的ip，这里假设1口），
        但与自己的某个端口（这里假设2口）处于同一网段，
        且该端口是局域网类型的（局域网才支持广播，如以太网，令牌环网），
        则会在2号端口上发起广播（但是广播的源mac替换为2口的mac），
        收到广播回复后，把arp响应包的源mac替换为1口的mac，
        然后发给1口上发起arp请求的计算机
    arp不响应的情况
        主机收到arp请求后，发现要查询的目标ip与自己的不符
        主机收到arp请求后，目标ip地址与自己匹配，但源ip地址与自己不在同一个网段，此时不会产生回应
        主机收到arp响应，但自己没发出arp请求，则丢弃收到的响应包
        路由器收到arp请求后，目标ip地址与自己匹配，但源ip地址与自己不在同一个网段，
        如果没有开启arp代理功能（思科默认开启），则不会产生回应
    ARP报文格式
        ----------------------┬----------------------------
             硬件类型         │          协议类型
        ---------┬------------┼----------------------------
