...
void* OpenTun(const char *ip, const char *mask, const char *dhcpservice)
{
	...
	ULONG ep[3] = {};
	//ep[0] = inet_addr(ip);
	InetPton(AF_INET, ip, (void*)& ep[0]);
	//ep[2] = inet_addr(mask);
	InetPton(AF_INET, mask, (void*)& ep[2]);
	ep[1] = ep[0] & ep[2];
...
int AddTunIpaddr(const char *ip, const char *mask)
{
	...
	ULONG NTEContext = 0;
	ULONG NTEInstance = 0;
	UINT IPAddress = 0;// inet_addr(ip);
	InetPton(AF_INET, ip, &IPAddress);
	UINT IPMask = 0;// inet_addr(mask);
	InetPton(AF_INET, mask, &IPMask);
...
int AddTunRoute(const char *ip, const char *mask, const char *gateway)
{
	...
	InetPton(AF_INET, ip, &fr.dwForwardDest);
	//fr.dwForwardMask = inet_addr(mask);
	InetPton(AF_INET, mask, &fr.dwForwardMask);
	//fr.dwForwardNextHop = inet_addr(gateway);
	InetPton(AF_INET, gateway, &fr.dwForwardNextHop);
...
BOOL ConfigDHCP(const char *ip, const char *mask, const char *DHCPServer)
{
	...
	InetPton(AF_INET, ip, &DhcpConfig[0]);
	InetPton(AF_INET, mask, &DhcpConfig[1]);
	InetPton(AF_INET, DHCPServer, &DhcpConfig[2]);
		...
		if (GetAdaptersInfo(pAdapterInfo, &ulOutBufLen) == NO_ERROR)
		{
			...
			while(pAdapter)
			{
				if (strncmp(ADAPTER_NAME, pAdapter->Description, strlen(ADAPTER_NAME)) == 0)
				{
					...
					InetPton(AF_INET, pAdapter->IpAddressList.IpAddress.String, &ip.S_un.S_addr);
					InetPton(AF_INET, pAdapter->IpAddressList.IpMask.String, &mask.S_un.S_addr);
