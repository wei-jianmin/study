...
public:
	...
	//创建ELEMENT节点，当k为NULL时，创建无名节点，否则为有名节点
	explicit JsonElement(const char* k="", JsonElement* v=NULL);
...
public:
	...
	JsonElement * get_element_bypath(const char* path);
	JsonElement * get_element_bypath(JsonElement* parent, const char* path);
...
public:
	...
	explicit JsonElementPtr(JsonElement* ptr){
		imp_ = ptr;
		if(imp_){
			imp_->ref_count++;
		}
