...
JsonElement::JsonElement(const char* k, const JsonElementPtr& v)
{
	if(k)
	{
		element_name = k;
...
void JsonElement::free_children()
{
	...
	while(!element_value.e.IsNull())
	{
		//JsonElement* p = element_value.e.Get();
		//delete p;
		//delete element_value.e;
		element_value.e.Release();
	}
}
*/
...
void JsonElement::set_value( JsonElementPtr& e )
{
	e->parent = this;
	element_value.e = e;
	element_value.b = false;
...
void JsonElement::detach()
{
	...
	{
		if(parent.IsNotNull() && parent->element_value.e==this)
...
{
	JsonElementPtr pe = get_child_at(index);
	pe = NULL;
}

void JsonElement::remove_all_children()
{
	while(element_value.e.IsNotNull())
		element_value.e.Release();
}

bool JsonElement::check_before_add_sibling(const JsonElementPtr& e)
{
	if(e==NULL)
	{
		throw_error(JsonError::PARAM_NULL,"param is null");	
...
JsonElementPtr JsonElement::set_child_jelement(const JsonElementPtr& e)
{
	if(e==NULL)
	{
		throw_error(JsonError::PARAM_NULL,"param is null");	
...
bool JsonElement::check_before_add_child(const JsonElementPtr& e)
{
	if(e==NULL)
	{
		throw_error(JsonError::PARAM_NULL,"param is null");	
...
//成功返回子节点，失败返回本节点
JsonElementPtr JsonElement::add_child_last(const JsonElementPtr& e)
{
	...
	{
		JsonElementPtr pe = this->element_value.e;
		e->prev = pe->prev;
		e->next = pe.Get();
		pe->prev = e;
		e->prev->next = e;
...
//成功返回子节点，失败返回本节点
JsonElementPtr JsonElement::add_child_first(const JsonElementPtr& e)
{
	JsonElementPtr pe = add_child_last(e);
	pe->parent->element_value.e = pe;
	return e;
}

JsonElementPtr JsonElement::add_sibling_before(const JsonElementPtr& e)
{
...
JsonElementPtr JsonElement::add_sibling_after(const JsonElementPtr& e)
{
	if(!check_before_add_sibling(e))
		return NULL;
	next->prev = e;
...
JsonElementPtr JsonElement::add_sibling_first(const JsonElementPtr& e)
{
	if(!check_before_add_sibling(e))
		return NULL;
	if(parent.IsNotNull())
	{
		JsonElementPtr pfirst = parent->element_value.e;	
		e->prev = pfirst->prev;
		e->next = pfirst;
		e->prev->next = e;
		pfirst->prev = e;
...
JsonElementPtr JsonElement::add_sibling_last(const JsonElementPtr& e)
{
	if(!check_before_add_sibling(e))
		return NULL;
	if(parent.IsNotNull())
	{
		JsonElementPtr plast = parent->element_value.e->prev;
		e->prev = plast;
		e->next = plast->next;
		plast->next = e;
		e->next->prev = e;
...
JsonElementPtr JsonElement::get_sibling_first()
{
	if(parent.IsNotNull())
...
JsonElementPtr JsonElement::get_sibling_at(int index)
{
	...
	JsonElementPtr p = get_sibling_first();
...
JsonElementPtr JsonElement::get_sibling_last()
{
	if(parent.IsNotNull())
...
JsonElementPtr JsonElement::get_root_element()
{
	JsonElementPtr p = parent;
	if(p == NULL)
		return this;
	while(p->parent.IsNotNull())
		p = p->parent;
	return p;
}

JsonElementPtr JsonElement::get_child_last()
{
	JsonElementPtr p = element_value.e;
	if(p==NULL)
	{
...
JsonElementPtr JsonElement::get_child_at( int index )
{
...
JsonHelper::JsonElementPtr JsonElement::get_element_bypath(const char* path)
{
	return get_element_bypath(get_root_element(),path);
}

JsonHelper::JsonElementPtr JsonElement::get_element_bypath(const JsonElementPtr& parent, const char* path)
{
	if(parent==NULL || path==NULL || strlen(path)==0 || path[0]!='/')
		return NULL;
	
	std::string name;
	JsonElementPtr first = parent->get_child_first();
	JsonElementPtr child = first;
...
{
	if(element_value.e.IsNotNull())
	{
		unsigned short count =1;
		JsonElementPtr p = element_value.e;
		JsonElementPtr p2 = p->next;
		while(p2!=p)
		{
			count++;
			p2=p2->next;
...
bool JsonElement::get_node_value(std::string & str_value,bool formated,int depth)
{
	...
	if(pele.IsNotNull())
	{
		...
		{
			JsonElementPtr pe = pele->element_value.e;
			if(pe == NULL)
			{
				str_value = "{ }";
...
bool JsonParser::dealwith_name_part(const JsonElementPtr& pe)
{
...
bool JsonParser::dealwith_bool_part(const JsonElementPtr& pe)
{
...
bool JsonParser::dealwith_num_part(const JsonElementPtr& pe)
{
...
bool JsonParser::dealwith_string_part(const JsonElementPtr& pe)
{
	if(json_value.at(index)!='"')
	{
		return false;
...
bool JsonParser::dealwith_ele_part(const JsonElementPtr& pe)
{
	if(json_value.at(index)!='{')
	{
		return false;
	...
	{
		JsonElementPtr pe2 = new JsonElement;
...
bool JsonParser::dealwith_array_part(const JsonElementPtr& pe)
{
	if(json_value.at(index)!='[')
	{
		return false;
...
JsonElementPtr JsonParser::new_jelement_from_json(std::string str_json)
{
	...
	JsonElementPtr pe = new JsonElement;
	try
	{
		pe->set_name("");
...
JsonElementPtr::JsonElementPtr(JsonElement* ptr)
{
	imp_ = ptr;
	if(imp_){
		imp_->AddRef();
...
JsonElementPtr::JsonElementPtr(const JsonElementPtr& ptr)
{
	imp_ = ptr.imp_;
	if(imp_){
		imp_->AddRef();
...
void JsonElementPtr::operator =(const JsonElementPtr& ptr)
{
	if(&ptr == this)
		return;
	if(imp_)
...
void JsonElementPtr::operator =(const JsonElement* ptr)
{
	if(ptr == imp_)
		return;
	if(imp_)
