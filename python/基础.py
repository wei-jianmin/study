...
数据类型
    ...
    数字类型（Number）
        ...
        通过内置type()函数，可以查看变量类型：
        x=3;print(type(x)) 输出结果为 : <class 'int'>
        通过isinstance(变量,类型) ，判断变量是否为指定类型或其子类(派生类)。
        python中的变量本质是引用，数字类型的变量引用的是Number（派生类的）实例
        py和Java一样，采用了享元模式的设计思想，
        每个对象都已一个唯一的id，用id(变量名)可得到该对象的id
        可以用del语句删除对象的引用，如 del var1,var2
        在混合运算时，py会把整形转为浮点型。
        py可以同时为多个变量赋值，如：a,b=1,2
        赋值二进制用0b开头，赋值8进制用0o开头，赋值十六进制用0x开头
        小整数池
    ...
    字符串(String)
        ...
        使用三引号可以指定一个多行字符串。
        py中的字符串只能读，不能修改。
        支持字符串自动拼接，如"this ""is"等价于"this is"
...
生成器
    ...
    举例：
        >>> def func3():
        ...     print("---1---")        #函数体第1句
        ...     yield 1                 #函数体第2句，把1赋值给generator对象，并返回该对象
        ...     print("---a---")        #函数体第3句
        ...     yield "a"               #函数体第4句，把'a'赋值给generator对象，并返回该对象
        ...     print("---[1,2]---")    #函数体第5句
        ...     yield [1,2]             #函数体第6句，把[1,2]赋值给generator对象，并返回该对象
        ...     print("---finish---")   #函数体第7句
        ...
